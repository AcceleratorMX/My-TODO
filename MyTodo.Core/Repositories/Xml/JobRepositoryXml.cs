...
namespace MyTodo.Core.Repositories.Xml;
...
public class JobRepositoryXml(XmlStorageService xmlStorageService) : IRepository<Job, int>
{
    public async Task<IEnumerable<Job>> GetAllAsync()
    {
        ...
        var jobs = document.Descendants("Job").Select(job => new Job
        {
            ...
            Deadline = !string.IsNullOrEmpty(job.Attribute("deadline")?.Value)
                ? DateTime.Parse(job.Attribute("deadline")!.Value)
                : null,
    ...
    public async Task<Job> GetByIdAsync(int id)
    {
        ...
        var job = new Job
        {
            ...
            Deadline = !string.IsNullOrEmpty(jobElement.Attribute("deadline")?.Value)
                ? DateTime.Parse(jobElement.Attribute("deadline")!.Value)
                : null,
    ...
    public async Task CreateAsync(Job job)
    {
        ...
        var newJob = new XElement("Job",
            ...
            new XAttribute("deadline", (job.Deadline != null ? job.Deadline : "")!),
